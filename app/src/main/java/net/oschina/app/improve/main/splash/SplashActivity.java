...
public class SplashActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        if (launcher != null && !launcher.isExpired() && file.exists()) {
            ...
            addFragment(R.id.fl_content, AdFragment.newInstance(launcher));   //显示广告页面
        }
    ...
    private void redirectTo() {
        if (OSCSharedPreference.getInstance().isFirstInstall()) {
            IntroduceActivity.show(this);   //第一次跳介绍页面
        } else {
            MainActivity.show(this);    //进入主界面
        }
        finish();
    }

    @Override
    public void onBackPressed() {   //停止返回键响应
        //super.onBackPressed();
    }
}
